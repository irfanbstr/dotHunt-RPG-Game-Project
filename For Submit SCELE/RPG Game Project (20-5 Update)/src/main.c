...
#include "textEngine.h"		//berisi metode-metode display teks
#include "gfxEngine.h"		//berisi metode display gambar
#include "controlEngine.h"	//berisi variabel dan metode kontrol keyboard

int job, progress; 			//Variabel permanen
char name[10] = "IRF";		//variable Nama karakter. Default = "IRF"


int main(void)				//main function
{				
	SetConsoleTitle("dotHunt by E&I"); //Judul program pada windows
	StartMenu(); //mulai startmenu
...
{ 
    ...
    while(start != 1){ //game belum mulai

		key = getch(); //get keycode dari metode getch()
    	switch(key){
    		case 13: //keycode untuk enter
    			start = 1; //mulai game
    			break;
...
{
	char password[10]; //variabel string password
	system("cls");
	text2("> Enter Password:\n\n",20);
	scanf("%s", password); // simpan string di variabel.
	printf("\n\n");
	
	if((strcmp(password, "beforethestorm") == 0)) // gunakan password "beforethestorm" untuk melihat prolog.
	{
...
int Disclaimer()			//teks disclaimer
{
	...
	text2("Semua aset gambar ASCII adalah milik www.asciiart.eu.\nKarya ini ditulis untuk kepentingan non-profit.\nPenulis tidak memiliki hak untuk menjual atau mengkomersialisasikan karya ini tanpa izin pihak yang bersangkutan.\n\n",10);
	system("pause");
}
...
int Prologue()				//berisi hardcode cerita prolog bagian 1
{
    ...
    speaking("", "news");
    Choice("Bagaimana Menurutmu?", "Ayo!", "Ke Arcade Game...?");
    
    if (choiceV == 1){
    	speaking("A", "news");
    	speak("Wah, kamu bersemangat sekali ya! Haha.");
    	speak("Hmm... Main apa ya disana...");
	}
	
	if (choiceV == 2){
    	speaking("A", "news");
    	speak("Hm? Sedang tidak mood untuk game?");
    	speak("Baiklah, mungkin kita pergi ke tempat lain dulu. Tapi setelah itu aku ingin mampir arcade game sebentar saja.");
	}
	
	speaking("", "news");
    speak("(Merasa tidak ada yang lebih baik untuk dilakukan, kamu setuju untuk pergi bersama A.)");
...
int Battle(char enemy[20])	//fungsi Battle, panggil dengan Battle("jenismusuh")
{
	
	system("cls");

	int PlayerHealth = 200;		//deklarasi variabel variabel dalam pertarungan
	int EnemyHealth = 100;		
	int Choice1,Choice2,Choice3;	//pilihan dalam menu
	int DmgToEnemy = 10;			//damage dari player ke musuh
	int DmgBlock=0;					//damage hasil blokir
	int FireDmg = 5;				//damage dari skill fire
	int IceDmg = 5;					//damage dari skill ice
	int ThunderDmg = 5;				//damage dari skill thunder
	int PMP = 100;					//player mp
	int EMP = 100;					//enemy mp
	srand(time(NULL));

	int x;	//randomizer untuk player
	int y;	//randomizer untuk musuh
	int burn = 0;	//status burn
	int frost= 0;	//status frost
	int shock = 0;	//status shock
	int DmgToPlayer;	//damage dari musuh ke player
	...
	do{
		...
		if (burn>0)
		{
			DmgToEnemy += burn; //tambah damage ke enemy berkat burn
			burn -= 1;
		...
		if (frost>0)
		{
			DmgToPlayer -= frost;	//kurangi damage ke player berkat frost
			frost -= 1;
		...
		if (shock>0)
		{
			DmgToPlayer = 0;		//player tdk terkena damage berkat shock
			shock -= 1;
		...
		{//the player choose to Skill
			printf("Skill list\n[1]Fire\n[2]Ice\n[3]Thunder\n");
			//The PLAYER then choose which skill to activate
			
			if (PMP >= 20)	//syarat MP diatas 20
			{
				PMP -= 20;
				printf("Choose skill: ");
				scanf("%d",&Choice2);
			...
			{//the player chose ice
				printf("================================================\n");
				printf("Player Menyerang dengan Frost\n %d Damage\n", IceDmg);
				EnemyHealth -= IceDmg;
				
				x = random % 2; // randomizer ice
				printf("%d\n",x);
			...
			{//the player choose thunder
				printf("============================================\n");
				printf("Player Menyerang dengan menggunakan Thunder\n %d Damage\n", ThunderDmg);
				EnemyHealth -= ThunderDmg;
					
				x = random % 2; //randomizer thunder
				...
				{
					printf("\nMusuh tidak terkena efek shock");//shock
				}
			}
